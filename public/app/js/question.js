$(document).ready(function() {
  ...
  $(".question-app-trac-nghiem .list-answer .answer").on("click", function(e) {
    var $this = $(this);
    $this.closest('.list-answer').find(".answer").removeClass("checked");
    ...
    function(e) {
      ...
      if ($this.hasClass("active")) {
        let data = {
          action :'bookmark',
          type: 2,
          question_id : $this.closest('.form_trac_nghiem').find('input[name="question_id"]').val()
        };
  ...
  $('.form_dien_tu .list-action .icon-sugess').on('click',function () {
    ...
    {
      $this.removeClass("active");
      $this.closest('.form_dien_tu').find('.sugess_all').hide();
    }else{
      $this.addClass("active");
      $this.closest('.form_dien_tu').find('.sugess_all').show();
    }
  });

  //hien goi y cho tung cau hoi
  $('.question-app-dien-tu .question-item .head .sugess').on('click',function () {
    var $this = $(this);
    if($this.hasClass("active"))
    {
      $this.removeClass("active");
      $this.closest('.question-item').find('.sugess_item').hide();
    }else{
      $this.addClass("active");
      $this.closest('.question-item').find('.sugess_item').show();
    }
  });
});
