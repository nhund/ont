                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <a href="{{ route('admin.category.add') }}" class="btn-primary btn">Thêm chuyên mục</a>
                        </div>
                        <div class="panel">
                                    ...
                                    @if($var['categories'])
                                        @foreach($var['categories'] as $key => $category)
                                            <tr class="tr">
                                                ...
                                                <td>{{ $key +1 }}</td>                                                
                                                <td>{{ $category->name }}</td>
                                                <td>{{ ($category->type == 1) ? 'Post' : 'News' }}</td>
                                                <td>{{ date('d-m-Y',$category->create_at ) }}</td>
                                                <td>
                                                    {{--<a href="#" class="btn btn-success btn-xs btn-label"><i class="fa fa-search"></i>Xem</a>--}}
                                                    <a href="{{ route('admin.category.edit',['id'=>$category->id]) }}" class="btn btn-default btn-xs btn-label"><i class="fa fa-pencil"></i>Sửa</a>
